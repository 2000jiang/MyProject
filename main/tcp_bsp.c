...
void recv_data(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (len > 0)
        {
            ...
            //接收数据回发
            // send(connect_socket,datall,strlen(databuff),0);
...
void rend_data(void *pvParameters)
{
    ...
    {
        //清空缓存
        memset(datall, 0x00, sizeof(datall));
        //读取接收数据
        strcpy(datall, distance);
        send(connect_socket, datall, strlen(datall), 0);
