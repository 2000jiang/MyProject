 ...
 */
int oled_write_cmd(uint8_t command) //写入数据
{
    //注释参考sht30之i2c教程
    int ret;
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();                                // i2c_cmd_handle_t相当于一个容器
    ret = i2c_master_start(cmd);                                                 //起始信号
    ret = i2c_master_write_byte(cmd, OLED_WRITE_ADDR | WRITE_BIT, ACK_CHECK_EN); //从机地址及读写位
